...
void main() {
  ...
  test('rust daemon ping responds with protocol metadata', () async {
  ...
  test('rust daemon watchOnce reports a file modification batch', () async {
    final client = RustDaemonClient(daemonDirectory: daemonDirectory);
    final tempDir = await Directory.systemTemp.createTemp('fbr-rust-daemon-test-');
    addTearDown(() async {
    ...
    if (response case RustDaemonErrorResponse(:final message)) {
      fail('Rust daemon returned an error payload instead of a watch batch: $message');
    }
    ...
    expect(batch.events.any((event) => event.path.endsWith('sample.txt')), isTrue);
    expect(
      batch.events.any(
